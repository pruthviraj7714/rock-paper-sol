...
import { connection, PLATFORM_WALLET_PUBLIC_KEY } from "@/lib/constants";
...
export default function GameClient() {
  ...
  const gameWinSoundRef = useRef<HTMLAudioElement>(null);
  const gameLoseSoundRef = useRef<HTMLAudioElement>(null);
  ...
  useEffect(() => {
    ...
    gameWinSoundRef.current = new Audio("/sounds/game-win.mp3");
    gameLoseSoundRef.current = new Audio("/sounds/game-lose.mp3");
  ...
  const handleMatchStart = async () => {
    ...
    playSound(clickSoundRef as React.RefObject<HTMLAudioElement>);
    try {
      ...
      if (res.data.success) {
        setStartMatch(true);
        playSound(gameStartSoundRef as React.RefObject<HTMLAudioElement>);
  ...
  const handleChoice = (choice: Choice) => {
    ...
    setTimeout(() => {
      ...
      if (result === "win") {
        playSound(winSoundRef as React.RefObject<HTMLAudioElement>);
        setPlayerScore((prev) => prev + 1);
      } else if (result === "lose") {
        playSound(loseSoundRef as React.RefObject<HTMLAudioElement>);
        setComputerScore((prev) => prev + 1);
      } else if (result === "draw") {
        playSound(drawSoundRef as React.RefObject<HTMLAudioElement>);
      }
  ...
  const handleGameResult = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setGameResult("win");
        playSound(gameWinSoundRef as React.RefObject<HTMLAudioElement>);
      } else {
        setGameResult("lose");
        playSound(gameLoseSoundRef as React.RefObject<HTMLAudioElement>);
      }
    } catch (error: any) {
      toast.error(error.response.data.message ?? error.message);
    } finally {
          ...
          <Card className="overflow-hidden border-0 bg-gray-800 shadow-xl">
            ...
            <CardContent className="flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="mb-8 text-xl text-gray-300">
                {gameResult === "win"
                  ? `Congratulations! You've won ${betAmount * 2} SOL!`
              ...
              {gameResult === "win" && (
                <span className="text-sm text-red-500 italic mt-4">
                  NOTE: 5% platform fee is automatically deducted from your
                  winnings.
                </span>
              )}
            </CardContent>
              ...
              <CardContent className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <div className="text-sm text-gray-400 bg-gray-700/50 rounded-full px-4 py-1">
                      First to 3 wins!
                    </div>
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 p-4">
                    <Square
                      className={`h-10 w-10 transition-transform duration-300 ${
                        startMatch ? "group-hover:scale-110" : ""
                      }`}
                    />
          ...
          <div className="w-80 rounded-2xl bg-gray-900 p-6 shadow-2xl border border-gray-800">
            <div className="flex flex-col items-center space-y-4">
              ...
              <h3 className="text-xl font-bold text-white">
                Initializing Your Match
              </h3>
              <p className="text-center text-gray-400">
                Please don&apos;t close or refresh the screen while we set up
                your game
              </p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-80 rounded-2xl bg-gray-900 p-6 shadow-2xl border border-gray-800">
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-center text-gray-400">
                Please don&apos;t close or refresh the screen while we verify
                your game
              </p>
            </div>
          </div>
